...
server.post("/tasks/", function(req, res, next) {
  ...
  if (error) {
    ...
    req.body.slug = createSlug(req.body.taskName); // Generate a slug for new tasks.
    next();
  }
});
...
function validateTask(task) {
  if (!task.taskName) return "Task Name is required.";
  if (!task.taskStatus) return "Task Status is required.";
  return "";
}
